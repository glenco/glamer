...
\begin{DoxyCompactList}\small\item\em Finds a concave envelope for an arbitrary closed curve. This is done by gridding and then finding points that are withing a sertain distance of a segment of the curve. The outer bounding curve is found and then the cuve is shrunck to the closest point on a segment. This should be fool proof, but is relatively slow and might clip some points. \end{DoxyCompactList}\item 
...
\begin{DoxyCompactList}\small\item\em returns true if x is within the polygon H \end{DoxyCompactList}\item 
